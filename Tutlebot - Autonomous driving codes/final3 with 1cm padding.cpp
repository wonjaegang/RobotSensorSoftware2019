...
static double destination_x = 2.4, destination_y = 0;//exampsle
...
void msgCALLBACK(const sensor_msgs::LaserScan::ConstPtr& scan)
{
	...
	for (int i = 0; i < 360; i++)//assumming the forward lidar angle is 0
	{
			...
			//if the obstacle locates inside of the rectengular map
		{
			if (!(lidarxy[0][i] == 0 && lidarxy[1][i] == 0))//filtering 0 values
			{
				...
				if(grid_x != 0 && grid_x != map_size - 1 && grid_y != 0 && grid_y != map_size - 1)
				for(int n = 0; n < 9; n++)
				{
					map[grid_x + (n / 3) - 1][grid_y + (n % 3) - 1] = 'x';
				}
			}
	...
	double g_head = 0.1, g_speed = 0.8, g_clear = 0.4;
	...
	for (int a = 0; a < dwa_size; a++)
	{
		for (int b = 0; b < dwa_size + 1; b++)
		{
			...
			if (d < turtlebot_length * 2 / 5) //preventing the selection of the v,w crossing over the obstacles
				break;
...
{
	q = asin(odom->pose.pose.orientation.z) * 2; //converting from quaternian to theta
